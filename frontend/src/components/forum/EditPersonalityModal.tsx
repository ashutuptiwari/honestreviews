// src/components/org/EditPersonalityModal.tsx
'use client';

import { useState, useEffect, FormEvent } from 'react';
import { useDispatch, useSelector } from 'react-redux';
import { AppDispatch, RootState } from '@/store';
import { updatePersonality, clearError } from '@/store/personalitySlice';
import { Personality, PersonalityUpdate } from '@/types/personalities';
...
export default function EditPersonalityModal({
  ...
  const [formData, setFormData] = useState<PersonalityUpdate>({
    name: '',
    description: '',
  });
    ...
    <div className="fixed inset-0 bg-black/50 dark:bg-black/70 flex items-center justify-center z-50">
      <div className="bg-light-surface dark:bg-dark-surface rounded-lg shadow-xl max-w-md w-full mx-4 border border-light-border dark:border-dark-border">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-light-border dark:border-dark-border">
          <h2 className="text-xl font-semibold text-light-text dark:text-dark-text">Edit Personality</h2>
          <button
            onClick={handleClose}
            disabled={loading}
        ...
        <form onSubmit={handleSubmit} className="px-6 py-4">
          ...
          {/* Name Field */}
          <div className="mb-4">
            <label htmlFor="edit-name" className="block text-sm font-medium text-light-text dark:text-dark-text mb-2">
            ...
            <p className="mt-1 text-xs text-light-text-secondary dark:text-dark-text-secondary">{formData.name?.length || 0}/100 characters</p>
          </div>

          {/* Description Field */}
          <div className="mb-6">
            <label htmlFor="edit-description" className="block text-sm font-medium text-light-text dark:text-dark-text mb-2">
              Description
            </label>
            <textarea
              id="edit-description"
