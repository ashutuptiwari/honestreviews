  ...
  return (
    <div className={`card hover:shadow-card-hover dark:hover:shadow-card-dark-hover transition-all duration-200 ${isJoined ? 'opacity-80' : ''}`}>
      <div className="flex justify-between items-start mb-3">
        <div className="flex-1 min-w-0">
          <Link href={`/orgs/${org.slug}`}>
            <h3 className="text-2xl font-semibold text-light-text-secondary dark:text-dark-text-secondary hover:text-light-primary dark:hover:text-dark-primary cursor-pointer transition-colors truncate">
              {org.name}
            </h3>
          </Link>
          {org.description && (
      ...
      <div className="flex items-center gap-3 mb-4 text-sm">
        <div className="flex items-center gap-1.5 bg-light-surface dark:bg-dark-surface px-3 py-1.5 rounded-full">
          <svg className="w-4 h-4 text-light-primary dark:text-dark-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
